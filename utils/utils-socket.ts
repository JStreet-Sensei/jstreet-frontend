import { Player, ServerLobby } from "@/types/game";
...
export function extractDataFromQuery(query: ParsedUrlQuery) {
  ...
  const lobby_id = Number(
    Array.isArray(lobby_id_q) ? lobby_id_q[0] : lobby_id_q,
  );
...
export function getPlayerFromSet(
  players: Set<Player> | undefined,
